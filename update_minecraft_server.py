...
version_manifest_url = 'https://launchermeta.mojang.com/mc/game/version_manifest.jso 
n' 
...
def update_latest_jar_symlink(jar_file_path): 
    ...
    # work around to overwrite any existing symlink (see https://stackoverflow.com/a 
/27788271) 
...
def main(): 
    ...
    if not latest_release:
        logging.error('Could not find latest release {} in the version manifest'.for
mat(latest_release_id))
        return 1
    ...
    if os.path.exists(jar_file_path):
        logging.debug('Already have latest Minecraft server located at {}'.format(ja
r_file_path))
        return 0
    ...
    logging.debug('successfully updated minecraft server, latest jar located at {}'.
